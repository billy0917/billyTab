...
const QuoteWidget: React.FC<QuoteWidgetProps> = ({ size }) => {
  ...
  return (
    <div className={`w-full h-full bg-white rounded-[2rem] text-gray-800 shadow-2xl relative overflow-hidden group hover:scale-[1.02] transition-transform duration-300 flex flex-col justify-between border border-white/50 ${isSmall ? 'p-5' : 'p-6'}`}>
      
      <div className="absolute -bottom-8 -right-8 w-32 h-32 bg-purple-100 rounded-full blur-2xl opacity-60"></div>
      
      {!isSmall && (
        <div className="absolute top-4 right-4 text-purple-500 opacity-20 transform rotate-12">
      ...
      <div className="relative z-10 h-full flex flex-col justify-center">
        <div>
          {!isSmall && <h3 className="text-[10px] font-bold text-purple-600 uppercase tracking-wider mb-2">Daily Insight</h3>}
          {loading ? (
            ...
            <p className={`${isSmall ? 'text-sm font-semibold line-clamp-4 leading-relaxed' : 'text-lg md:text-xl font-medium leading-snug line-clamp-3'} text-gray-900`}>
              "{data?.quote}"
            </p>
          )}
        </div>
        ...
        {!loading && data && !isSmall && (
          <div className="mt-3">
            <p className="text-sm text-gray-500 font-medium">— {data.author}</p>
          </div>
        )}
      </div>
    </div>
